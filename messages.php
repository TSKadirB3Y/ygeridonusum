...
if (!empty($search_query)) {
    $users_stmt = $pdo->prepare("SELECT id, first_name, last_name FROM users WHERE id != ? AND (first_name LIKE ? OR last_name LIKE ?)");
    $users_stmt->execute([$current_user_id, "%$search_query%", "%$search_query%"]);
    $users = $users_stmt->fetchAll(PDO::FETCH_ASSOC);
}

// Mesajlaşılmış kişileri listeleme
...
<body>
    <div class="sidebar">
        ...
        <form action="messages.php" method="POST">
            <input type="text" name="search_user" placeholder="Kullanıcı adı ara..." value="<?php echo htmlspecialchars($search_query); ?>">
    ...
    <div class="chat">
        <?php if ($chat_user_id && $chat_user): ?>
            ...
            <form action="messages.php?chat=<?php echo $chat_user_id; ?>" method="POST">
                <textarea name="message_content" placeholder="Mesajınızı yazın..." required></textarea>
