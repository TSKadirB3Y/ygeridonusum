...
if (isset($_POST['follow_action']) && $user_id != $_SESSION['user_id']) {
    if ($is_following) {
        // Takibi bırak
        $unfollow = $pdo->prepare("DELETE FROM follows WHERE follower_id = ? AND following_id = ?");
        $unfollow->execute([$_SESSION['user_id'], $user_id]);
        $is_following = false;
        $followers_count--;
    } else {
        // Takip et
        $follow = $pdo->prepare("INSERT INTO follows (follower_id, following_id) VALUES (?, ?)");
        $follow->execute([$_SESSION['user_id'], $user_id]);
        $is_following = true;
        $followers_count++;
    }
}
...
<head>
    ...
    <style>
        ...
        .post-modal-comments .comment-content {
            flex: 1;
            background: #f8f9fa;
            padding: 10px;
            border-radius: 10px;
        ...
        .post-modal-comments .comment-username {
            font-weight: 600;
            margin-bottom: 5px;
            color: var(--text-color);
        }

        .post-modal-comments .comment-text {
            margin: 0;
            color: var(--text-color);
            word-wrap: break-word;
        }

        /* Yorum scrollbar stilini özelleştir */
...
<body>
            ...
            <?php else: ?>
                    <div class="profile-actions">
                        <form method="POST" style="display: inline;">
                            <button type="submit" name="follow_action" class="follow-btn <?php echo $is_following ? 'following' : 'not-following' ?>">
        ...
        <div class="post-modal-content">
            ...
            <div class="post-modal-right">
                ...
                <div class="post-modal-actions">
                    <button type="button" class="post-action like-button" id="modalLikeButton" onclick="likePost(this)" style="background: none; border: none; padding: 0; cursor: pointer;">
                        <i class="far fa-heart"></i>
                        <span class="likes-count">0</span>
                    </button>
                    <button class="post-action" style="background: none; border: none; padding: 0; cursor: pointer;">
                        <i class="far fa-comment"></i>
                ...
                <div class="post-modal-actions">
                    <form id="commentForm" class="mt-3">
                        ...
                        <div class="input-group">
                            <input type="text" class="form-control" name="comment_content" placeholder="Yorum yaz..." required>
    ...
    <script>
        function showPostModal(postId) {
            const post = <?php echo json_encode($user_posts); ?>.find(p => p.id === postId);
            if (!post) return;

            const modal = document.getElementById('postModal');
            const mediaContainer = document.getElementById('postModalMedia');
            ...
            likeButton.querySelector('.likes-count').textContent = post.likes_count;
            
            // Yorum sayısını ayarla
            commentsCount.textContent = post.comments_count;
            
            // Medya içeriğini ayarla
            mediaContainer.innerHTML = '';
            if (post.image_path) {
                mediaContainer.innerHTML = `<img src="${post.image_path}" alt="Post image" class="post-modal-image">`;
            } else if (post.video_path) {
                mediaContainer.innerHTML = `
                    <video controls class="post-modal-video">
        ...
        // Yorum gönderme işlemi için event listener ekle
        document.getElementById('commentForm').addEventListener('submit', function(event) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    const mainPagePost = document.querySelector(`#post${postId}`);
                    if (mainPagePost) {
                        const mainPageCommentCount = mainPagePost.querySelector('.post-action i.fa-comment').nextElementSibling;
                        mainPageCommentCount.textContent = currentCount + 1;
                    }
                } else {
        ...
        function likePost(button) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    const mainPageButton = document.querySelector(`.post-card[id="post${postId}"] .like-button`);
                    if (mainPageButton) {
                        mainPageButton.querySelector('.likes-count').textContent = data.likes_count;
                        if (data.is_liked) {
                            mainPageButton.querySelector('i').classList.remove('far');
                            mainPageButton.querySelector('i').classList.add('fas');
                            mainPageButton.classList.add('liked');
                        } else {
                            mainPageButton.querySelector('i').classList.remove('fas');
                            mainPageButton.querySelector('i').classList.add('far');
